 ...
 * ログインまわり。
 *
 * 小さなチームで使う前提なので、ログインID＋パスワードだけの軽い作りにしている。
 * パスワードは password_hash() で保存し、生の文字列はどこにも残さない。
 * ログイン状態は「推測できないトークン」を HttpOnly の Cookie に入れて持ち回る。
 */
...
final class Auth
{
    public const COOKIE = 'mikata_session';
    ...
    {
        $db  = Db::conn();
        $sel = $db->prepare('SELECT * FROM users WHERE login = ?');
        $sel->execute([trim($login)]);
        $row = $sel->fetch();

        // ID が無いときも password_verify を通して、応答時間で存在を悟られないようにする。
    ...
    public static function startSession(int $userId): string
    {
        ...
        $db    = Db::conn();
        $ins   = $db->prepare('INSERT INTO sessions (token, user_id, created_at, seen_at) VALUES (?, ?, ?, ?)');
