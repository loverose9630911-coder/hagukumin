...
アイコンにも中身と同じ意味を持たせています。**外側のリングがチーム全体の進み具合**、
**真ん中の点がその中にいる自分**、**リングの切れ目がまだ終わっていないぶん**。
ダッシュボードのドーナツと同じ形なので、アイコンと画面が地続きになります。
...
├── tools/render-icons.mjs アイコンを作り直す（make icons）
└── tests/run.php        テスト
```

アイコンは `tools/render-icons.mjs` にある図形1つを大もとにして、SVG と PNG
（ホーム画面用・iOS 用・古いブラウザ用）をまとめて書き出しています。`make icons` で作り直せます。
