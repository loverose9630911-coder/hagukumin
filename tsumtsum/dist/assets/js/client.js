...
(function () {
  ...
  function post(url, body) {
    // 静的版（オフライン PWA）はサーバーの代わりにブラウザ内エンジンを使う
    if (window.LocalEngine && window.LocalEngine.enabled) {
      return LocalEngine.request(url, body || {});
    }
    return fetch(url, {
      method: 'POST',
  ...
  function showResult(data) {
    ...
    if (typeof window.__game.onFinish === 'function') window.__game.onFinish(data);
  }
  ...
  function boot() {
    ...
    bindSoundToggle();
    requestAnimationFrame(loop);
    // SPA（静的版）では、タイトルの「ゲームスタート」を押してから始める
    if (!window.SHELL_MODE) startGame();
  }

  /** 1 プレイぶんの状態を作り直して開始する */
  function startGame() {
    G.phase = 'loading';
    G.tsums = [];
    G.chain = [];
  ...
  window.__game = {
    ...
    sendChain: sendChain,
    startGame: startGame,
    onFinish: null            // SPA 側が結果を受け取りたいときに差しこむ
  };
